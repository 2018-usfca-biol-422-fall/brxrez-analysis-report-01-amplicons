...
**Figure 2**: Alpha diversity measure of two samples, colored in by sample source. Raw observed data is compared to Shannon index of the two specific samples. 
...
```
**Figure 3**: Inferred phylogeny of existing phylum solely on space bar sample type. Tree represents phylogeny inferred using FastTree. Phylum branches are displayed from short to long. 
...
plot_tree(subset_deinococcus_thermus,
  ...
  ggtitle("")
```
**Figure 4**: Deinococcus-thermus presence in space bar samples and its presence in descendant phylum. 
...
plot_tree(small_obj, 
          color = "Sex",
          ladderize = TRUE)+
  ggtitle("Deinococcus thermus in males and females")+


```
**Figure 5**: Presence of deinococcus thermus in male and female subjects.  
