...
ggtitle("Alpha Diversity Measure of Space Bar and Finger Tip")
  
```

**Figure 2** Alpha diversity measure of two samples, colored in by sample source. 

number of different strains (actual number) and shannon counts evenenness and richness.

```{r melting phyloseq}
melted_phyloseq <- psmelt(phyloseq_obj)
melted_phyloseq %>%
  filter(sample_source %in% c("Space_bar", "finger_tip")) %>%
  group_by()
```
